...
export const makingReport = async (url) => {
  try {
    ...
    const after_rate = isNaN(data[10][2]) ? 0 : data[10][2];
    const befor_rate = isNaN(data[9][2]) ? 0 : data[9][2];
    const diff = Math.round((after_rate - befor_rate) * 100) / 100;
    const report = `대량보유상황보고 
    보고자명: ${data[6][3]}
    회사명: ${data[6][1]}
    보유량변화: ${befor_rate}% → ${after_rate}% (${diff}%)
    보고사유: ${data[20][1]}
    `.trim();
    return report;
  } catch (err) {
